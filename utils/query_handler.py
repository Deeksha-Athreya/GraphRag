...
def query_graph(graph, query):
    ...
    print(f"Query Entities: {query_entities}")  # Debug: Show entities extracted from query

    # Score nodes based on their relevance to the query
    relevant_nodes = set()
    for node in graph.nodes:
        for query_ent in query_entities:
            if query_ent in node:
...
def generate_response(query, subgraph):
    ...
    print(f"Subgraph Text: {subgraph_text}")  # Debug: Output subgraph text
